...
def _clean_numeric_col(series: pd.Series) -> pd.Series:
    """Vectorized number cleaning."""
    # Remove commas, treat '-', '–' as NaN
    s = series.astype(str).str.strip().str.replace(",", "")
    s = s.replace({"-": np.nan, "–": np.nan, "—": np.nan, "": np.nan, "None": np.nan})
    # Handle negative numbers in parentheses e.g. (1000) -> -1000
...
def _parse_mixed_dates(series: pd.Series) -> pd.Series:
    """Handles columns that mix 'DD-MM-YYYY' strings and Excel Serial floats."""
    s_str = series.astype(str).str.strip()
    
    # 1. Try standard date parsing (Day First)
    dates_std = pd.to_datetime(s_str, dayfirst=True, errors='coerce')
    
    # 2. Identify failed rows that might be Excel Serials (numbers > 10000)
    # We look for rows where dates_std is NaT but the original string is numeric
    mask_nat = dates_std.isna()
    numeric_part = pd.to_numeric(s_str[mask_nat], errors='coerce')
    
    # Filter for valid Excel date range (approx 1927 to 2119)
    valid_serials = numeric_part[(numeric_part > 10000) & (numeric_part < 80000)]
    
    # Convert serials to datetime
    dates_serial = pd.to_datetime(valid_serials, unit='D', origin='1899-12-30')
    
    # 3. Combine
    final_dates = dates_std.copy()
    final_dates.update(dates_serial)
...
# CACHING ADDED HERE (TTL = 10 minutes)
@st.cache_data(ttl=600, show_spinner="Fetching Trust Data...")
def load_reit_ndcf(url: str, sheet_name: str = TRUST_SHEET_NAME) -> pd.DataFrame:
    ...
    # Numeric conversions
    numeric_cols = [COMP_COL, DECL_INCL_COL, CFO_COL, CFI_COL, CFF_COL, PAT_COL]
...
@st.cache_data(ttl=600, show_spinner="Fetching SPV Data...")
def load_reit_spv_ndcf(url: str, sheet_name: str = SPV_SHEET_NAME) -> pd.DataFrame:
    ...
    str_cols = ["Name of REIT","Financial Year","Period Ended","Name of SPV","Name of Holdco (Leave Blank if N/A)"]
    for c in str_cols:
        if c in df.columns:
            df[c] = df[c].astype(str).str.strip()
...
@st.cache_data(ttl=3600) # Cache offer docs longer
...
def compute_trust_timeline_checks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Logic: Record date should be shortly after Declaration, Distr shortly after Record
...
def compute_spv_checks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Valid if the GAP is less than the total computed amount (loose check)
...
def render():
    ...
    with tab_trust:
        ...
        if q.empty:
            ...
            st.caption("Checking if (CFO + CFI + CFF + PAT) matches Computed NDCF within 10%")
            disp2 = qc[[
            ...
            if not tline.empty:
                ...
                st.warning("Date columns missing or invalid.")
