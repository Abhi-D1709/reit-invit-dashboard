...
def evaluate_meetings_for_committee(
    ...
    min_indep = rules["min_meetings"] and rules.get("min_indep_present", None)
    ...
    if m.empty:
        ...
        # Add gap row only if a gap rule exists for this committee
        if rules.get("gap_days") is not None:
            summary_rows.append(
                {
                    "Rule": "Max gap between meetings (days)",
                    "Expected": rules["gap_days"],
                    "Observed/Status": "—",
                }
            )
    ...
    if gap_days_rule is not None:
        gap_text = (
            f"Worst gap: {worst_gap} (OK≤{gap_days_rule})"
