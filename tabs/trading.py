...
st.set_page_config(page_title="REIT / InvIT • NSE/BSE Charts", layout="wide")

# ============================== Google Sheet ==============================
SHEET_ID = "1g44Lkv3VZU4FDTzrWXKhdxGwrWecZHHrZLmuRMyFHDI"
...
def aggregate_volume_and_turnover(df: pd.DataFrame, monthly: bool) -> pd.DataFrame:
    ...
    - If monthly=True: one row per month (bar per month),
      * volume = sum(volume)
            ...
            try:
                ...
                if j.get("error"):
                    ...
                    if "No record" in msg or "No records" in msg:
                        return empty_df()
                    # If server complains about >1y, we'll split via windows anyway
    ...
    if mode == "Single Entity":
        ...
        if nse_all.empty and bse_all.empty:
            ...
            tabs = st.tabs([
                ...
                "BSE & NSE (per entity)",          # NEW
                "Aggregated Volume – BSE & NSE",   # NEW
            ])
...
st.markdown(
    """
**Notes**
- **NSE fetch hardened for Streamlit Cloud**: browser-like headers, session cookie warm-up, 4× retries, backward ≤1y windows, and a one-time monthly gap refill.
- **Monthly mode**: volume=sum per month; close=last trading day of that month. Bar labels are positioned diagonally under each month.
- **Group View**: new combined tabs — *BSE & NSE (per entity)* and *Aggregated Volume – BSE & NSE*.
"""
)
