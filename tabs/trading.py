...
def get_nse_data(symbol: str, series: str, start: dt.date, end: dt.date) -> pd.DataFrame:
    ...
    # 1. SPECIAL HEADERS TO BYPASS NSE BLOCKING
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    ...
    # 2. SERIES SAFETY (Check RR, IV, EQ just in case)
    target_series = list(set([series, "RR", "IV", "EQ", "BE"]))
    series_json = json.dumps(target_series)

    # 3. USE SESSION TO PERSIST COOKIES (Critical)
    session = requests.Session()
    session.headers.update(headers)

    try:
        # Visit homepage first to look like a human
    ...
    def fetch_chunk(d1, d2):
        ...
        try:
            ...
            # If still blocked, print error to logs
            ...
            required = ["CH_TIMESTAMP", "CH_CLOSING_PRICE", "CH_TOT_TRADED_QTY"]
    ...
    # 4. CHUNK REQUESTS (180 days max to be safe)
    CHUNK = 180
    windows, cur = [], end
    while cur >= start:
        s = max(start, cur - dt.timedelta(days=CHUNK))
        windows.append((s, cur))
        cur = s - dt.timedelta(days=1)
    
    parts = [fetch_chunk(s, e) for s, e in windows]
    valid = [p for p in parts if not p.empty]
    
    if not valid: return pd.DataFrame(columns=["date", "close", "volume"])
    
    return pd.concat(valid, ignore_index=True).drop_duplicates(subset=["date"]).sort_values("date").reset_index(drop=True)
...
def render():
    ...
    if mode == "Single Entity":
        ...
        with st.status(f"Fetching daily data for {len(rows_to_fetch)} {group_type}s...", expanded=True) as status:
            with ThreadPoolExecutor(max_workers=8) as executor:
                ...
                for _, r in rows_to_fetch.iterrows():
                    # FIX: Add ScriptRunContext to avoid thread warnings
                    f = executor.submit(fetch_single_entity, r, start_date, end_date, False, use_cache_now)
        ...
        with tabs[3]:
            ...
            else: st.dataframe(combined_all_daily.sort_values(["Entity", "date", "Exchange"]), width="stretch", hide_index=True)
        ...
        with tabs[0]:
            ...
            if combined_all_daily.empty: st.info("No data to aggregate.")
                ...
                st.plotly_chart(volume_only_bar(agg_df, f"Total Monthly Volume (BSE+NSE) • All {group_type}s", monthly=True), width="stretch")
        ...
        with tabs[1]:
            ...
            if daily_nse_all.empty: st.info("No NSE data to aggregate.")
                ...
                st.plotly_chart(volume_only_bar(agg_df, f"Total Monthly Volume (NSE) • All {group_type}s", monthly=True), width="stretch")
                st.dataframe(agg_df, width="stretch", hide_index=True)
        with tabs[2]:
            ...
            if daily_bse_all.empty: st.info("No BSE data to aggregate.")
                ...
                st.plotly_chart(volume_only_bar(agg_df, f"Total Monthly Volume (BSE) • All {group_type}s", monthly=True), width="stretch")
                st.dataframe(agg_df, width="stretch", hide_index=True)