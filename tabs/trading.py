    ...
    if mode == "Single Entity":
        ...
        with st.status(f"Fetching daily data for {len(rows_to_fetch)} {group_type}s...", expanded=True) as status:
            with ThreadPoolExecutor(max_workers=8) as executor:
                ...
                for _, r in rows_to_fetch.iterrows():
                    # FIX: Submit task and attach Streamlit context to prevent 'ScriptRunContext' warnings
        ...
        with tabs[3]:
            ...
            if combined_all_daily.empty: st.info("No daily data to display.")
                ...
                # FIX: Replaced use_container_width=True with width="stretch"
                st.dataframe(combined_all_daily.sort_values(["Entity", "date", "Exchange"]), width="stretch", hide_index=True)
