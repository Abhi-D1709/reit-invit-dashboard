...
def get_nse_data(symbol: str, series: str, start: dt.date, end: dt.date) -> pd.DataFrame:
    """
    Revised NSE fetcher with 180-day chunks to strictly avoid API limits,
    plus advanced headers to bypass cloud blocking.
    """
    if start > end: return pd.DataFrame(columns=["date", "close", "volume"])
    
    # ADVANCED HEADERS: Mimic a real Chrome browser
    headers = {
    ...
    # Check multiple series to be safe (IV, RR, EQ, BE)
    ...
    try:
        ...
        pass # Proceed even if homepage fails
    ...
    # CHUNK LOGIC: Split into 180-day blocks (Safe limit)
    CHUNK_DAYS = 180
    windows = []
    current_end = end
    ...
    # Reverse to fetch in chronological order if desired, though concat handles it
